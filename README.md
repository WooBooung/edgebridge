...
> **라즈베리파이 / 시놀로지 NAS 호환 멀티 아키텍처 Docker 이미지** 를 Docker Hub 로 자동 배포합니다.
...
| 멀티 아키텍처 Docker 이미지 자동배포 | 수동 빌드 | ✅ **GitHub Actions → Docker Hub (amd64/arm64)** |
...
**기본은 host 네트워크입니다.** (실제 LAN IP 인식 + mDNS 자동 발견 + MQTT/허브 포워딩이 모두 동작 → 포트 매핑 혼란 없음)
...
```
- 포트 매핑이 없고 호스트의 `Server_Port`(기본 **8088**)로 직접 수신 → `http://<host-ip>:8088`.
- PAT 넣으려면 `-e EB_ST_TOKEN=<36자 PAT>` 추가.
- 웹 대시보드: `http://<host-ip>:8088/dashboard`

### docker compose
저장소의 [`docker-compose.yml`](docker-compose.yml)(host 네트워크) 사용:
```sh
docker compose up -d
```

> 컨테이너는 데이터(`/data`)에 `.registrations`, `redirects.jsonl`, `callbacks.jsonl`, `mqtt_certs/`, `install_id` 를 영속 저장합니다.
> **포트 격리가 필요하면** bridge 네트워크 버전 [`docker-compose.bridge.yml`](docker-compose.bridge.yml) (또는 `-p 8088:8088`)을 쓰세요 — 단 mDNS·자동 IP 인식은 안 됩니다. ([🌐 네트워크 모드](#-네트워크-모드--mdns-자동-발견) 참고)
...
### 설치 순서 (방법 A — Container Manager GUI, 권장 · host 네트워크)
...
5. **네트워크 단계 → "Docker 호스트와 동일한 네트워크 사용" 선택** ← 이게 핵심(host). **포트 설정은 안 합니다.** (호스트의 8088 로 직접 수신)
6. **볼륨(스토리지) → 폴더 추가**: `/docker/edgebridge-aeb/data` → 마운트 경로 **`/data`**
   - PAT 등 설정은 **환경 변수**(`EB_ST_TOKEN`)나 cfg 마운트로 — 아래 [🔑 PAT 등 설정 적용하기](#-pat-등-설정-적용하기-환경-변수-또는-cfg-마운트) 참고.
7. **적용 → 완료** → 실행 → `http://<NAS-IP>:8088` 접속.

> **왜 host 인가:** 클라이언트 실제 IP 인식 + mDNS 자동 발견 + MQTT/허브 포워딩이 모두 동작하고, 컨테이너-포트 vs 로컬-포트 혼란이 없습니다.
> 8088 이 NAS 에서 충돌하면 환경 변수 `EB_SERVER_PORT` 로 바꾸세요.
>
> **포트 격리가 꼭 필요하면(bridge):** 네트워크를 bridge 로 두고 **포트 설정**에서 `로컬(원하는값) : 컨테이너 8088(고정)` 매핑 — 단 mDNS·자동 IP 인식은 안 됩니다.
...
이 이미지는 **host 네트워크가 기본**입니다. 브리지의 **일부 기능은 "나에게 연락한 상대(허브)의 진짜 LAN IP"** 를 알아야 하기 때문입니다:
- **device→hub 포워딩**, **MQTT 메시지 허브 전달**, **mDNS 자동 발견(`_edgebridge._tcp`)**.

Docker **bridge 네트워크**로 띄우면 들어오는 요청의 출발지가 도커 게이트웨이(`172.17.0.1`)로 보여 위 기능들이 동작하지 않습니다.
(반면 `/api/forward` 같은 요청·응답형 기능은 **영향 없음**.) 그래서 **host 를 기본**으로 합니다.
...
docker run -d --name edgebridge-aeb \
  ...
  woobooung/edgebridge-aeb:latest
```
- 포트 매핑(`-p`)이 없습니다. 호스트의 `Server_Port`(기본 **8088**)로 직접 수신 → `http://<호스트-IP>:8088`.
- 8088 충돌 시 환경 변수 `EB_SERVER_PORT`(또는 cfg `Server_Port`)로 변경.
- compose: 기본 [`docker-compose.yml`](docker-compose.yml) 이 곧 host 입니다 → `docker compose up -d`.
- 시놀로지 GUI: 위 [설치 순서](#설치-순서-방법-a--container-manager-gui-권장--host-네트워크) 5번에서 **"Docker 호스트와 동일한 네트워크 사용"** 선택.

### bridge 네트워크 (포트 격리가 꼭 필요할 때만)
- compose [`docker-compose.bridge.yml`](docker-compose.bridge.yml), 또는 `docker run ... -p <로컬포트>:8088`.
- ⚠️ 이 모드에선 클라이언트 IP가 `172.17.0.1`로 보여 **mDNS·device→hub·MQTT 자동 IP 가 동작하지 않습니다.** forward 등록 시 주소를 직접 명시해야 합니다.
...
> 대안: 분리망이거나 host 모드를 못 쓰면 **macvlan** 으로 브리지에 자체 LAN IP 를 부여하는 방법도 있습니다 (Docker macvlan 네트워크를 직접 구성).

---

## 🖥️ 웹 대시보드
브리지와 같은 포트에서 `GET /dashboard` 로 간단한 운영 대시보드를 제공합니다.

- 상태 요약: 브리지 버전, 시작 시간, SmartThings PAT 설정/검증 상태, mDNS 상태
- 목록 확인: MQTT 세션, 등록된 디바이스 포워딩, redirect, callback
- 관리 작업: redirect 추가/삭제, callback 저장/삭제, `SmartThings PAT` / `forwarding_timeout` / `mDNS_enabled` / `mDNS_name` 변경
- JSON 요약 API: `GET /api/dashboard`
...
```jsonc
{
  ...
  "bridgeDevice": "server",
  "bridgeVersion": "1.0.0_AEB",
  "serverStartTime": "06/23 14:30",
  "supportedAiOptions": [],       // LLM 미이식
  "stOauthConnected": true,       // cfg 의 PAT 가 유효하면 true (아래 설명)
  "stTokenConfigured": true,      // PAT 가 cfg 에 설정되어 있는지
  "stTokenValid": true,           // 그 PAT 가 SmartThings 에서 유효한지
  "mqtt": { "total": 1, "connected": 1, "sessions": [ {"id":"sess_...","state":"CONNECTED","lastError":null} ] },
  "blocked": { "hosts": 0, "attempts": 0 }
}
```
- **PAT 유효성**: `edgebridge.cfg` 에 `SmartThings_Bearer_Token` 이 설정돼 있으면, 브리지가 SmartThings API
  (`GET /v1/locations`)를 호출해 토큰이 유효한지 확인합니다. 401 이 아니면 유효로 보고 `stOauthConnected`/`stTokenValid` 를 `true` 로 표시합니다.
  (매 ping 마다 호출하지 않도록 **5분간 캐시**. PAT 미설정 시 모두 `false`.)
...
이 포크는 **`main` 브랜치에 push 하면 GitHub Actions 가 멀티 아키텍처 이미지를 빌드해 Docker Hub 로 자동 배포**합니다
